...
function compute_fft(signal, dt)
    ...
    # Используем rfft для вещественных сигналов
    # (возвращает только положительные частоты)
...
if !isnan(peak_time_prey) && !isnan(peak_time_predator)
    ...
    println("\nАнализ колебаний:")
    println(
        "Первый пик жертв: время = ",
        round(peak_time_prey, digits = 2),
        ", значение = ",
        round(peak_value_prey, digits = 2)
    )
    println(
        "Первый пик хищников: время = ",
        round(peak_time_predator, digits = 2),
        ", значение = ",
        round(peak_value_predator, digits = 2)
    )
...
function analyze_parameter_sensitivity(param_index, values, param_name)
    ...
    for val in values
        ...
        println(
            " $(param_name)=$(val): жертвы=$(round(prey_end, 2)), хищники=$(round(predator_end, 2))"
        )
    end
