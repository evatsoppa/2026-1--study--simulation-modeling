...
plot!(plt7[1], df_ode.t, df_ode.S, label=L"S(t)", linewidth=2, title="Восприимчивые")
plot!(plt7[2], df_ode.t, df_ode.I, label=L"I(t)", linewidth=2, title="Зараженные")
plot!(plt7[3], df_ode.t, df_ode.R, label=L"R(t)", linewidth=2, title="Выздоровевшие")
plot!(plt7[4], df_ode.t, df_ode.I, label=L"I(t)", linewidth=2, yscale=:log10, title="Лог. масштаб")
plot!(plt7[5], df_ode.S, df_ode.I, label=false, linewidth=2, title="Фазовый портрет", xlabel=L"S", ylabel=L"I")
plot!(plt7[6], df_ode.t, df_ode.Re, label=L"R_e", linewidth=2, title=L"R_e(t)", hline=[1.0], linestyle=:dash, linecolor=:red)
